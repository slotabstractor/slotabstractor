...
parser.add_argument('--model_name', type=str, default='slot_attention_random_spatial_heldout_unicodes_resizedcropped_pretrained_frozen_autoencoder_abstractor_scoring')
...
abstractor_model = scoring_model(args,args.hid_dim,args.depth,args.heads,args.mlp_dim,args.num_slots).to(device)
slot_model = load_slot_checkpoint(slot_model,'slot_attention_autoencoder_6slots_clevrdecoder_morewarmup_lowerlr_nolrdecay_64dim_128res_random_spatial_heldout_unicodes_resizedcropped_continuetraining_run_1_best.pth.tar')
...
# params = [{'params': list(slot_model.parameters()) + list(abstractor_model.parameters())}]
params = [{'params': abstractor_model.parameters()}]
...
for epoch in range(1,args.num_epochs+1):
	...
	for batch_idx, (img,target) in enumerate(train_dataloader):
		...
		for idx in range(img.shape[1]):
			...
			# feat_slots,pos_slots = slot_model(img[:,idx],device)
		...
		scores = abstractor_model(feat_panels,pos_panels,device)
...
# 		del recons, masks, slots
	...
	# 		# print(torch.max(img), torch.min(img),img.shape)
			...
			for idx in range(test_img.shape[1]):
				...
				# test_feat_slots, test_pos_slots = slot_model(test_img[:,idx],device)
				...
				# del test_feat_slots,test_pos_slots
